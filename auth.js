...
async function loadProfilePage() {
  ...
  if (!profileView || !editForm) return;
  ...
  try {
    ...
    if (displayNickname) displayNickname.textContent = `@${me.username}`;
    if (displayRole) displayRole.textContent = me.role || 'Программист';

    const avatarSrc = me.avatar || '../img/user.png';
    if (displayAvatar) displayAvatar.src = avatarSrc;
    if (editAvatarPreview) editAvatarPreview.src = avatarSrc;
    if (editNickname) editNickname.value = me.username || '';
    ...
    editForm.addEventListener('submit', async (e) => {
      ...
      try {
        const payload = {
            username: editNickname?.value.trim() || me.username,
            avatar: avatarDataUrl || me.avatar || null
        };
        ...
        if (displayRole) displayRole.textContent = updated.role || 'Программист';
